...
def getA(filename):
	...
	for i in xrange(0,len(procdata)):
...
def ALS(A):
	...
	# create H and W initial from the average value of A
	avg = avrat(A)
	...
	# for jefff in xrange(0,3):
		...
		prin = np.dot(H,np.transpose(W))
		# print(prin.astype(int))
		# print('max=',np.max(prin))
